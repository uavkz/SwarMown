...
CX = {"ox": cx_order, "pmx": cx_pmx, "cx": cx_cycle}
MUT = {"swap": mut_swap, "insert": mut_insert, "inversion": mut_inversion}
...
def tsp_ga(M, cx_name, mut_name, pop_size, ngen, mutation_chance, opt_cost, cxpb=0.5, mutpb=1.0):
    ...
    def fit(ind):
        return tour_cost(M, ind[0])

    pop = [[random.sample(range(n), n)] for _ in range(pop_size)]
    best_cost = min(fit(ind) for ind in pop)
    gen_hit = None
    ...
    for g in range(ngen):
        ...
        fits = [fit(o) for o in offspring]
        gen_best = min(fits)
        if gen_best < best_cost:
            best_cost = gen_best
        if gen_hit is None and best_cost - opt_cost <= tol:
            gen_hit = g + 1
        # tournament selection (size 3) into next generation
        nxt = []
        for _ in range(pop_size):
            a, b, c = random.randrange(pop_size), random.randrange(pop_size), random.randrange(pop_size)
            w = min((a, b, c), key=lambda k: fits[k])
            nxt.append([offspring[w][0][:]])
        pop = nxt
...
def main(seeds=30, pop=50, ngen=60, mutation_chance=0.1):
    ...
    roles = ["C3line", "C3tri", "C5mixed", "C5holes", "C5varied", "C10grid"]  # N >= 3
