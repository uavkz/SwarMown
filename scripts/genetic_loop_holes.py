...
for mission in Mission.objects.filter(id__in=[29, 30, 31, 32]).exclude(field__holes_serialized="[]"):
    ...
    os.system(
        f"python -m scoop -n {N_CORES} scripts\\genetic_holes.py --mission_id {mission_id} --ngen {ngen} --population_size {population_size} --filename {filename} --max-time {max_time} --borderline_time {borderline_time} --max_working_speed {max_working_speed} --mutation_chance {mutation_chance}"
    )
